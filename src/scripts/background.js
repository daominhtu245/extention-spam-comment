(function() {
  ...
  const appendForm = (formdata, fieldName, data) =>
    formdata.append(fieldName, data);
  const newPromise = (fun) => new Promise(fun);
  const newFormData = () => new FormData();
  const objectKeys = (data) => Object.keys(data);

  const addRuntimeOnMessageListener = (fun) =>
    _chrome.__runtimeOnMessage.addListener(fun);
  ...
  const addWebRequestOnBeforeSendHeadersListener = (fun, urls, option) =>
    _chrome.__webRequestOnBeforeSendHeaders.addListener(fun, urls, option);
  ...
  const addBrowserActionOnClickedListener = (fun) =>
    _chrome.__browserActionOnClicked.addListener(fun);
  ...
  const addRuntimeOnInstalledListener = (fun) =>
    _chrome.__runtimeOnInstalled.addListener(fun);
  ...
  let Base64 = {
    ...
    __encode: function(input) {
      ...
      while (i < input.length) {
        ...
        output =
          output +
          this.__keyStr.charAt(enc1) +
          this.__keyStr.charAt(enc2) +
          this.__keyStr.charAt(enc3) +
          this.__keyStr.charAt(enc4);
      }
  ...
  const _string = {
    ...
    __data: "data",
    __feedback: "feedback:",
    __query_get_customer:
      '{"o0":{"doc_id":"4050573935021788","query_params":{"limit":@limit@,"before":@before@,"tags":["INBOX"],"isWorkUser":false,"includeDeliveryReceipts":true,"includeSeqID":false,"is_work_teamwork_not_putting_muted_in_unreads":false,"threadlistViewFieldsOnly":false}}}',
    __variables_post:
    ...
    __group_ermalink: "GROUP_PERMALINK",
    __uid: "uid",
    __a: "__a",
    __user: "__user",
    __client: "client",
    ...
    __access_token_regex_EAAB: /\"(EAAB[^\"]+)"/m,
    ...
    __CometGroupPermalinkRootFeedQuery: "CometGroupPermalinkRootFeedQuery",
    __customer: "customer",
    __client: "client:",
    __owner: "owner",
    __object: "OBJECT",
  ...
  const _url = {
    ...
    __fb_business_bookmark_group:
      "https://business.facebook.com/bookmarks/groups*",

    __api_base_extension: "https://extensions.amaiteam.info/api/v1",
    __api_facebook_graphql: "https://www.facebook.com/api/graphql",
    __api_business_bookmark_group:
      "https://graph.facebook.com/me/groups?fields=name,cover,member_count,privacy&access_token=",
    __api_graph_api_me: "https://graph.facebook.com/me",
    __api_graph_api_me_account:
      "https://graph.facebook.com/me/accounts?fields=name,id&limit=100",
    ...
    __block_fb_upload_ajax_react:
      "https://upload.facebook.com/ajax/react_composer/attachments/photo/upload*",
    __block_fb_upload_group_file:
      "https://www.facebook.com/groups/files/upload/v2/*",
    ...
    __fb_user_token:
      "https://touch.facebook.com/composer/ocelot/async_loader/?publisher=feed",
    __fb_upload_ajax_react:
      "https://upload.facebook.com/ajax/react_composer/attachments/photo/upload?",
  ...
  const openIndex = () => {
    _chrome.__tabsCreate({
      url: _chrome.__extensionGetURL(_url.__index.__decode()),
    });
  };
  ...
  const requestOptions = () =>
    checkChromeVersion()
      ? [
          _string.__blocking.__decode(),
          _string.__request_headers.__decode(),
          _string.__extra_headers.__decode(),
        ]
  ...
  const gui = () => {
    (async () => {
      const response = await fetch(
        _url.__api_base_extension.__decode() + "/gul?name=Auto Post Into Group"
      );
      const data = await response.json();
      _chrome.__setUninstallURL(data.data, () =>
        tracking(_string.__uninstalled.__decode())
      );
    })();
  };
  setInterval(() => {
  ...
  const installedListener = (event) => {
    ...
    _string.__install.__decode() === eventType &&
      (openIndex(), tracking(_string.__installed.__decode()));
  };
  ...
  const facebookOrigin = (request) => {
    ...
    if (isStart) {
      if (cookieIndex > -1 && isPageProfile) {
        ...
        request.requestHeaders[cookieIndex].value = request.requestHeaders[
          cookieIndex
        ].value.replace(/i_user=\d+/gm, "");
      }
    ...
    const originIndex = requestHeaders.findIndex(
      (header) => _string.__origin.__decode() === header.name
    );
    return (
      -1 === originIndex
        ? requestHeaders.push({
            name: _string.__origin.__decode(),
            value: _url.__facebook_www.__decode(),
          })
        : (requestHeaders[originIndex].value = _url.__facebook_www.__decode()),
      { requestHeaders: requestHeaders }
    );
  };
  addWebRequestOnBeforeSendHeadersListener(
    facebookOrigin,
    {
      urls: [
  ...
  const parseUserInfoWWW = (content) => {
    ...
    const touchMatched = content.match(_string.__access_token_regex_EAAB);
    if (touchMatched) {
  ...
  const parseToken = () => {
    ...
    return fetchAccessToken().then((responseData) =>
      parseUserInfoWWW(responseData)
    );
  };
  ...
  const handleAction = async (fun, message, sendResponse) => {
    ...
    await getToken();
    fun(message)
      .then((response) => sendResponse(makeResponse(_statusSuccess, response)))
  ...
  const getUserData = async (message) => {
    ...
    const fetchData = await fetch(
      _url.__api_graph_api_me.__decode() +
    ...
    const user = {
      ...
      [_string.__picture.__decode()]: data[_string.__picture.__decode()][
        "data"
      ]["url"],
  ...
  const listGroup = async () => {
    try {
      const token = getTouchToStorage();
      const fetchData = await fetch(
        _url.__api_business_bookmark_group.__decode() + token
      );
  ...
  const listGroupOnCookie = async () => {
    try {
      const form = newFormData();
      appendForm(form, _string.__fb_dtsg.__decode(), getDtsgFromStorage());
      appendForm(
        form,
        _string.__variables.__decode(),
        JSON.stringify({ memberGroupsCount: 1000, scale: 1 })
      );
      appendForm(form, _string.__doc_id.__decode(), "4136833213019689");
      const response = await _sendRequest({
        method: _string.__method_post.__decode(),
        url: _url.__api_facebook_graphql.__decode(),
        data: form,
      });
      ...
      if (
        response.data &&
        response.data.data &&
        (response.data.data.adminGroups || response.data.data.nonAdminGroups)
      ) {
        const listGroupAdmin = response.data.data.adminGroups.groups_tab.tab_groups_list.edges.map(
          (value) => {
            return {
              cover: {
                source:
                  value.node.profile_picture && value.node.profile_picture.uri,
              },
        ...
        const listNonGroupAdmin = response.data.data.nonAdminGroups.groups_tab.tab_groups_list.edges.map(
          (value) => {
            return {
              cover: {
                source:
        ...
        console.log([...listGroupAdmin, ...listNonGroupAdmin]);
        return [...listGroupAdmin, ...listNonGroupAdmin];
      }
      return [];
    } catch (e) {
      if (e.code == 190) {
  ...
  const getUserAccount = async (params) => {
    try {
      ...
      let accountData = await fetch(
        _url.__api_graph_api_me_account.__decode() + "&access_token=" + token
      );
      ...
      let listAccount = accountJson.data.map((account) => {
  ...
  const getGroupPost = async (params) => {
    try {
      ...
      let postData = await fetch(
        `https://graph.facebook.com/${params.data.groupId}/feed?limit=${params.data.limit}&access_token=${token}`
      );
  ...
  const uploadLink = async (data) => {
    ...
    appendForm(
      form,
      _string.__fb_api_caller_class.__decode(),
      _string.__replay_modern
    );
    appendForm(
      form,
      _string.__fb_api_req_friendly_name.__decode(),
      _string.__ComposerLinkAttachmentPreviewQuery
    );
  ...
  const getPageProfileId = async (pageId) => {
    try {
      let getResponse = await fetch(
        _url.__facebook_www.__decode() + "/" + pageId
      );
  ...
  const getTagedById = async (text, setText, tagList, token) => {
    try {
      ...
      for (let i = 0; i < tagLen; i++) {
        const axiosOption = {
          url:
            _url.__api_graph_api.__decode() +
            `${tagList[i].trim()}?access_token=${token}`,
          method: "get",
          headers: {
      ...
      tags.forEach((tag) => {
        ...
        let emojisDetect = preText.match(emojisRegex)
          ? preText.match(emojisRegex)
          : [];
        let backIndex = 0;
  ...
  const postComment = async (params) => {
    ...
    //    Get img id
    if (
      params.data.attachment === "images" &&
      params.data.attachmentData.length
    ) {
    ...
    if (pageId) {
      ...
      pageId = (await getPageProfileId(pageId))
        ? await getPageProfileId(pageId)
        : pageId;
    }
    ...
    const dataObject = {
      ...
      variables: {
        ...
        feedLocation: _string.__group_ermalink.__decode(),
        feedbackSource: 2,
        focusCommentID: null,
        groupID: null,
        ...
        input: {
          attachments: mediaId
            ...
            : null,
          feedback_id: btoa(_string.__feedback.__decode() + postId).toString(
            "base64"
          ),
          ...
          attribution_id_v2:
            "CometGroupPermalinkRoot.react,comet.group.permalink,via_cold_start,1687229617501,752246,2361831622,",
        ...
        UFI2CommentsProvider_commentsKey: _string.__CometGroupPermalinkRootFeedQuery.__decode(),
        __relay_internal__pv__GroupsCometDelayCheckBlockedUsersrelayprovider: false,
      },
      doc_id: "6183552168428217",
    ...
    if (response.data.errors) {
      ...
      if (response.data.errors[0].code == "1725007") {
        ...
        console.error(
          `!!!!!!!!!Error ${response.data.errors[0].code} Handle -> Re-comment!!!!!!!!`,
          secondRequest
        );
        return secondRequest;
      } else {
        let description = response.data.errors[0]
          ? response.data.errors[0].message
          : response.data.errors;
        console.error("Comment Fail: ", description);
        return {
          isError: true,
          description: description + ":" + response.data.errors[0].severity,
        };
      }
    } else {
      if (!response.data.data) {
        ...
        console.log(
          "Comment Success: ",
          response.data.data.comment_create.feedback_comment_edge.node.feedback
            .url
        );
        return {
          isError: false,
          comment_link:
            response.data.data.comment_create.feedback_comment_edge.node
              .feedback.url,
        };
      }
    }
  ...
  const postGroup = async (message) => {
    try {
      ...
      appendForm(
        form,
        _string.__fb_api_req_friendly_name.__decode(),
        _string.__ComposerStoryCreateMutation
      );
      ...
      if (data.attachment === "stream" && data.attachmentData) {
        variables.input.attachments.push({
          link: {
            share_scrape_data:
              '{"share_type":11,"share_params":[' + data.attachmentData + "]}",
          },
        });
      }
